//! Dense feature-matrix I/O.
//!
//! Input is a tab-separated `n×d` matrix: `n` samples (rows) by `d` features
//! (columns). A leading empty top-left cell marks a header row, and then the
//! first column holds row names; otherwise every cell is data and rows/columns
//! are numbered from 1. Cells equal to `NA`/`NaN`/empty parse to `f64::NAN`;
//! `Inf`/`-Inf` parse to infinities. Numeric cells go through `fast-float2`.
...
impl Matrix {
    ...
    /// Materialise column `j` into a fresh `Vec` (row-major storage strides a
    /// column). Used by the per-feature fit.
    #[must_use]
    ...
    /// Read a matrix from a file (`-` or `None` = stdin) and parse it.
    ///
    /// # Errors
    /// Errors on a missing file, an empty input, a ragged body, or a
    /// non-numeric cell.
    ...
    /// Parse a tab-separated matrix. A header is detected when the first
    /// non-blank line begins with a tab (empty top-left cell). Comment (`#`)
    /// and blank lines are skipped.
    ///
    /// # Errors
    /// Errors on an empty input, a ragged body, or a non-numeric cell.
...
/// Shortest round-trip decimal, scientific for very small or very large
/// magnitudes; `NA`/`Inf`/`-Inf` for non-finite values.
#[must_use]
...
mod tests {
    ...
    #[test]
    fn parse_na_inf() {
        let m = Matrix::parse("1\tNA\tInf\n-Inf\t2\t3\n").unwrap();
        assert!(m.row(0)[1].is_nan());
        assert_eq!(m.row(0)[2], f64::INFINITY);
        assert_eq!(m.row(1)[0], f64::NEG_INFINITY);
    }
    ...
    #[test]
    fn fmt_tokens() {
        assert_eq!(fmt_value(f64::NAN), "NA");
